...
glm::vec3 obj::getColorAtCoord( glm::vec4 texturecoord,unsigned char* texturePtr,int width,int height){
    

	float x = fmod(1.0f+texturecoord.x,1.0f);
	float y = fmod(1.0f+texturecoord.y,1.0f);
	int row = (int)(x * width);
    int col = (int)(y * height);
	int index = (row + col*width);

	glm::vec3 result;
	result.x = (float)(texturePtr[index]) / 255.0;
	result.y = (float)(texturePtr[index + 1]) / 255.0;
	result.z = (float)texturePtr[index + 2] / 255.0;
...
void obj::buildTBO(){
	...
	for(unsigned int k = 0; k<faces.size(); k++){
		...
		if(isConvex(faces[k])==true){
			vector<int> facetext = facetextures[k];
			for(int i=2; i<facetext.size(); i++){

				TBOvec.push_back(texturecoords[facetext[0]][0]); TBOvec.push_back(texturecoords[facetext[0]][1]); TBOvec.push_back(texturecoords[facetext[0]][2]); //TBOvec.push_back(1.0f);
				TBOvec.push_back(texturecoords[facetext[i-1]][0]); TBOvec.push_back(texturecoords[facetext[i-1]][1]); TBOvec.push_back(texturecoords[facetext[i-1]][2]); //TBOvec.push_back(1.0f);
				TBOvec.push_back(texturecoords[facetext[i]][0]); TBOvec.push_back(texturecoords[facetext[i]][1]); TBOvec.push_back(texturecoords[facetext[i]][2]); //TBOvec.push_back(1.0f);
...
void obj::setColorBy2DTexture( unsigned char* texturePtr,int width,int height){
	...
	for(unsigned int k = 0; k<faces.size(); k++){
		...
		if(isConvex(faces[k])==true){
			...
			for(int i=2; i<face.size(); i++){
				vector<int> facetext = facetextures[k];
				glm::vec3 color0=getColorAtCoord(texturecoords[facetext[0]],texturePtr,width,height);
				glm::vec3 color1=getColorAtCoord(texturecoords[facetext[i-1]],texturePtr,width,height);
				glm::vec3 color2=getColorAtCoord(texturecoords[facetext[i]],texturePtr,width,height);
				CBOvec.push_back(color0[0]); CBOvec.push_back(color0[1]); CBOvec.push_back(color0[2]); //CBOvec.push_back(1.0f);
				CBOvec.push_back(color1[0]); CBOvec.push_back(color1[1]); CBOvec.push_back(color1[2]); //CBOvec.push_back(1.0f);
				CBOvec.push_back(color2[0]); CBOvec.push_back(color2[1]); CBOvec.push_back(color2[2]); //CBOvec.push_back(1.0f);
	...
	cbo = new float[CBOvec.size()];
	cbosize = (int)CBOvec.size();
	for(int i=0; i<CBOvec.size(); i++){
		cbo[i] = CBOvec[i];
	}
}
